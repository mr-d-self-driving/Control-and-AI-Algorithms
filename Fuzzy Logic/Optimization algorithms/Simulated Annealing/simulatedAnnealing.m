...
while T > ParSA.Tmin
	...
	% implementeer hier de keuze van een punt uit de omgeving van de regelbank
        rij = randi(aantalx1);
        kolom = randi(aantalx2);
        nieuweRegelbank = besteRegelbank;
        nieuweRegelbank(rij, kolom) = besteRegelbank(rij, kolom) + randsample(omgeving, 1);
        if nieuweRegelbank(rij,kolom) > aantaly;
            nieuweRegelbank(rij,kolom) = aantaly;
        elseif nieuweRegelbank(rij,kolom) <= 0;
            nieuweRegelbank(rij,kolom) = 1;
        end  
